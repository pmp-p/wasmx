...
int  main  (int i_argc, const char* i_argv[])
{
    ...
    fprintf(stdout, " WASM_MEMORY_LIMIT : %d KiB =================\n", WASM_MEMORY_LIMIT / 1024 );
    if ( init_ram(d_m3FixedHeap) ) {
        CLOG("heap at %p", dynHeapPtr);

    } else {
        CLOG("m3_NewRuntime failed : no heap test: %p", malloc(d_m3FixedHeap));
        return 1;
    }
