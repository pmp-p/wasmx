...
//"wasi_unstable",
    static const char* namespaces[1] = { "wasi_snapshot_preview1" };
CLOG("m3_LinkWASI-2");
    // fd_seek is incompatible
    //m3_LinkRawFunction (module, "wasi_unstable",          "fd_seek",     "i(iIi*)", &m3_wasi_unstable_fd_seek);
    m3_LinkRawFunction (module, "wasi_snapshot_preview1", "fd_seek",     "i(iIi*)", &m3_wasi_snapshot_preview1_fd_seek);



    for (int i=0; i< LENGTH(namespaces); i++)
    {
        const char* wasi = namespaces[i];
CLOG("m3_LinkWASI-3 %d %s",i, wasi);
...
#include "extra/m3_api_wasi.extra"
...
m3_LinkWASI(IM3Module module) {
    ...
    m3_LinkRawFunctionEx (module, wasi, "proc_exit",          "v(i)",    &m3_wasi_generic_proc_exit, wasi_context);
    WASI_EXTRA
    CLOG("m3_LinkWASI %s done", wasi);
    return result;
}
